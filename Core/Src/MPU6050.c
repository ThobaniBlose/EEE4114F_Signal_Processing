 ...
 * ─────────────────────────────────────────────────────────────────────────────
 */
#define ACCEL_SENSITIVITY   8192.0f    /* LSB/g   at ±4g range    */
...
void MPU6050_Init(void)
{
    ...
    if (check == 104)  /* 104 = 0x68 */
    {
        ...
        /* ── Step 5: Accelerometer range → ±4g ──────────────────────────────
         * Writing 0x08 to ACCEL_CONFIG register:
         *   Bits [4:3] = AFS_SEL = 01 → ±4g range, 8192 LSB/g.
         * Avoids clipping during wave motion while keeping good resolution. */
        data = 0x08;
